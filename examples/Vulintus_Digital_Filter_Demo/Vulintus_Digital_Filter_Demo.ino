...
// Digital filter. //
// const Vulintus_Filter_Type FILTER_TYPE = HIGHPASS;              // Shorthand for "SINGLE_POLE_HIGHPASS".
const Vulintus_Filter_Type FILTER_TYPE = LOWPASS;               // Shorthand for "SINGLE_POLE_LOWPASS".
...
const float PRINT_FREQ = 10;            // Print frequency, in Hz.
...
void loop() {
    ...
    // On each scheduled print, print the current filter output.
    if (millis() > next_print) {                        // If it's time to print the filter output...
        Serial.print("RAW:");                           // Raw signal label.         
        Serial.print(test_filter.last_input,3);         // Print the most recent filter input.
